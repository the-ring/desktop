...
const webpackElectronConfig = require('./webpack.config.electron');
...
gulp.task('css', () =>
  ...
  .pipe(postcss([
    ...
    require('autoprefixer')({ browsers: ['Chrome >= 51'] })
...
gulp.task('webpack:electron', callback =>
  webpack(webpackElectronConfig, (err, stats) => {
    if (err) {
...
gulp.task('build', ['webpack:build', 'webpack:electron']);
